...
const Header = ({ isLoggedIn, onLogout }: HeaderProps) => {
    ...
    <div id='header' className={`${isDarkSection ? 'theme-dark' : 'theme-light'} ${isMenuOpen ? 'menu-open' : ''}`}>
        ...
        <nav className={`nav_container ${isMenuOpen ? 'active' : ''}`}>
            ...
            <ul className='gnb2'>
            <div className="header_left_fixed">
            ...
            <li className={`menu_item ${openSubMenu === 'user' ? 'active' : ''}`} onClick={() => toggleSubMenu('user')}>
                ...
                <img src={isDarkSection ? "/media/etc/userL_w.svg" : "/media/etc/userL_b.svg"} alt="user" />
                </Link>
            ...
            <li className='menu_item pc_only_search'>
        ...
        {/* 모달 검색창 유지 */}
        {isSearchOpen && (
            <div className={`search_modal_overlay ${isSearchOpen ? 'active' : ''}`} onClick={handleCloseSearch}>
            <div className='search_modal_content' onClick={(e) => e.stopPropagation()}>
                <div className='search_input_wrapper'>
                <input type='text' autoFocus={isSearchOpen} value={searchValue} onChange={(e) => setSearchValue(e.target.value)} />
        ...
        <button className="mobile_menu_btn" onClick={() => setIsMenuOpen(!isMenuOpen)}>
            <img src={isDarkSection ? "/media/etc/arrow_w.svg" : "/media/etc/arrow_b.svg"} alt="menu toggle" />
        </button>
        </div>
